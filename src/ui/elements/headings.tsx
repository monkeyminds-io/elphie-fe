...
export const Heading = ({children, styles = '', level, title}: HeadingProps) => {
    if(level === 1) return <h1 className={`font-bold text-gray-800 text-4xl capitalize md:text-5xl lg:text-6xl ${styles}`} title={title}>{children}</h1>
    ...
    if(level === 4) return <h4 className={`text-md font-semibold text-gray-800 ${styles}`} title={title}>{children}</h4>
}